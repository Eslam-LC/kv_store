...
- **Startup order**: `WAEngine.Init` creates missing files, loads the snapshot, replays `wal.log` on top, **then** catalogs SSTables newest-first. Replay applies records to KVS as it reads — a corrupt WAL leaves the store partially loaded (temp-holder + commit-on-success would make it atomic; deferred).
