...
- REPL commands: `put`, `get`, `delete`, `scan <start> <end>`, `snapshot save|load`, `replay`, `exit`. Values are UTF-8 text; hex via `-x`/`--hex` on `put`/`get`/`scan` (`put -x` tokens are plain hex, no `0x` prefix).
...
- **Startup order**: `WAEngine.Init` creates missing files, loads the snapshot, replays `wal.log` on top, **then** catalogs SSTables newest-first. **Replay is atomic**: `ReplayRecords` copies the current memstore into a temp store, reads the WAL into that same store, and swaps `MemStore` by reference only on full success — a corrupt WAL aborts at the first bad frame with nothing applied. Load snapshot is likewise read into `tempDict` before `BulkInitialize`. See `Replay_CorruptWALTail_*` tests.
...
- **Compaction = full flatten, auto-triggered, tombstone-pruning site.** When `immutableSSTables.Count > CompactThreshold` (4) at the end of a flush, `CompactFlat` folds the whole catalog newest-first into one table — the same `AddWithoutUpdate` first-wins merge as `Scan` — dropping all tombstones (safe only because a full flatten leaves nothing older below). Winner reuses the **oldest** serial via `File.Move(tmp, final, overwrite:true)`, then stragglers are `File.Delete`d *after* the move commits; catalog = single winner. Gotchas: (1) the delete pass must skip the winner by `FileName != finalPath` — the oldest source shares the winner's name, so a naive loop deletes the just-committed file; (2) tmp must live in `FilesPath` (`Path.Combine`), not CWD. Pin tests: `WAEngineTests.Flush_OverThreshold_CompactsToSingleTable`, `WAEngineTests.Compact_ThenRestart_SingleTableOnDisk`.
